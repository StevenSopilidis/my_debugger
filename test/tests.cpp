...
namespace {
    ...
    std::int64_t get_entry_point_offset(std::filesystem::path path) {
        ...
        // get file_address of entry point (_start)
...
TEST_CASE("Read registers works", "[register]") {
...
TEST_CASE("Can crearte breakpoint site", "[breakpoint]") {
    auto proc = process::launch("targets/run_endlessly");
...
TEST_CASE("Can find breakpoint site", "[breakpoint]") {
    ...
    // test both const and non-const functions
...
TEST_CASE("Hardware breakpoint evades memory checksums", "[breakpoint]") {
    ...
    proc->resume();
    proc->wait_on_signal();
    REQUIRE(to_string_view(channel.read()) == "Doing something good...\n");
    ...
    REQUIRE(to_string_view(channel.read()) == "Doing something evil...\n");
}
...
TEST_CASE("Correct DWARF language", "[dwarf]") {
    ...
    REQUIRE(compile_units.size() == 1); // only hello_sdb.cpp
...
TEST_CASE("Itearate DWARF", "[dwarf]") {
    auto path = "targets/hello_sdb";
    sdb::elf elf(path);
    auto& compile_units = elf.get_dwarf().compile_units();
...
TEST_CASE("Multi-threading works", "[threads]") {
    ...
    proc.resume_all_threads();
    reason = proc.wait_on_signal();
    REQUIRE(reason.reason == sdb::process_state::exited); // main thread exited
    close(dev_null);
}