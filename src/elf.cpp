...
void sdb::elf::parse_section_headers() {
    ...
    if (n_headers == 0 and header_.e_shentsize != 0) {
        // to many sections, num of them stored in sh_size field of the first section header
...
std::string_view sdb::elf::get_section_name(std::size_t index) const {
    auto& section = section_headers_[header_.e_shstrndx]; // section string table
...
std::optional<sdb::file_addr> sdb::elf::get_section_start_addr(std::string_view name) const {
    if (auto sect = get_section(name); sect) {
