...
namespace {
    ...
    sdb::line_table::file parse_line_table_file(
        ...
        if (file[0] != '/') {
            if (dir_index == 0) { // relative path, relative to compilation directory
                path = compilation_dir / std::string(file);
            } else { // relative to include dirs
                path = include_dirs[dir_index - 1] / std::string(file); // include dirs begin at 1
            }
        }
    ...
    std::unique_ptr<sdb::line_table> parse_line_table(const sdb::compile_unit& cu) {
        ...
        if (version != 4)
            sdb::error::send("Only DWARF4 is supported");

        (void)cur.u32(); // header length
        
        auto min_instruction_length = cur.u8();
        if (min_instruction_length != 1)
            sdb::error::send("Invalid minimum instruction length");

        auto max_operations_per_instruction = cur.u8();
        ...
        std::array<std::uint8_t, 12> expected_opcode_lengths { // 12 standard opcodes
            0, 1, 1, 1, 1, 0, 0, 0, 1, 0, 0, 1 // expected operands per standard opcode
        };
        ...
        return std::make_unique<sdb::line_table>(
            data, &cu, default_is_stmt, line_base,
    ...
    parse_abbrev_table(const sdb::elf& obj, std::size_t offset) {
        ...
        do
        {
            ...
            std::vector<sdb::attr_spec> specs;
            std::uint64_t attr = 0;
            ...
            do
            {
                ...
                if (attr != 0) {
                    specs.push_back({attr, form});
                }
            } while (attr != 0);
            
            if (code != 0) {
                table.emplace(code, sdb::abbrev{code, tag, has_children, std::move(specs)});
            }
...
sdb::dwarf::compile_unit_contains_address(file_addr address) const {
...
sdb::compile_unit::compile_unit(
    dwarf& parent, sdb::span<const std::byte> data, std::size_t abbrev_offset
...
void sdb::dwarf::index() const {
    // check if index has already been populated
...
sdb::die sdb::compile_unit::root() const {
    std::size_t header_size = 11; // size of compile unit header
...
sdb::file_addr sdb::die::low_pc() const {
    if (contains(DW_ATE_address)) {
        // ranges are always in ascending order of address
        auto first_entry = (*this)[DW_ATE_address].as_range_list().begin();
...
std::optional<std::string_view> sdb::die::name() const {
    ...
    if (contains(DW_AT_specification)) { 
        // name defined from header file, DW_AT_specification points to 
        // original DIE
        return (*this)[DW_AT_specification].as_reference().name();
    }

    if (contains(DW_AT_abstract_origin)) {
        // inlined functoins, DW_AT_abstract_origin points to original
        // copied function
...
sdb::die::children_range::iterator::operator++() {
    if (!die_.has_value() or !die_->abbrev_) return *this; // its null

    if (!die_->abbrev_->has_childen) {
        cursor next_cu({die_->next_, die_->cu_->data().end()});
        die_ = parse_die(*die_->cu_, next_cu);
    } 
    else {
        // skip children
...
sdb::die sdb::attr::as_reference() const {
    ...
    case DW_FORM_ref_udata:
        ...
        break;
    case DW_FORM_ref_addr: {
        // can reference into other compile units thus offset is relative to 
        // the start of .debug_info
        offset = cur.u32();
        ...
        auto cu_finder = [=](auto& cu) {
            return cu_->data().begin() <= die_pos and cu_->data().end() > die_pos;
        };
...
sdb::range_list::iterator::operator++() {
    ...
    while (true)
    {
        ...
        if (current_.low.addr() == base_address_flag) { // base_address found
            base_address_ = current_.high;
        }
        else if (current_.low.addr() == 0 and current_.high.addr() == 0) { // end-of-list indicator
            pos_ = nullptr;
            break;
        }
        else { // regular entry 
...
bool sdb::die::contains_address(file_addr address) const {
    if (address.elf_file() != cu_->dwarf_info()->elf_file()) {
        return false;
    }
...
sdb::line_table::iterator sdb::line_table::line_table::begin() const {
    return iterator(this);
}

sdb::line_table::iterator sdb::line_table::line_table::end() const {
    return {};
}
...
sdb::line_table::iterator::operator++() {
    ...
    } while (!emitted); // while instruction does not emit new row of matrix
    
    // subtract one since files are indexed from 1
...
bool sdb::line_table::iterator::execute_instruction() {
    ...
    if (opcode > 0 and opcode < table_->opcode_base_) {
        ...
        case DW_LNS_const_add_pc:
            registers_.address += 
                (255 - table_->opcode_base_) / table_->line_base_;
            break;
...
sdb::line_table::get_entries_by_address(std::filesystem::path path, std::size_t line) const {
