...
namespace {
    ...
    public:
        ...
        std::uint64_t uleb128() {
            ...
            {
                byte = u8();
                auto masked = static_cast<std::uint64_t>(byte & 0x7f); // mask first bit
                res |= masked << shift;
                shift += 7;
            } while ((byte & 0x80) != 0); // while first bit is not zero (there is continuation)
        ...
        // skips attribute from abbrev entry
