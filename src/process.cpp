...
namespace {
    void set_ptrace_options(pid_t pid) {
        if (ptrace(PTRACE_SETOPTIONS, pid, nullptr, 
            ...
            sdb::error::send("Failed to set TRACESYSGOOD and TRACECLONE options");
        }
    }
    ...
    int find_free_stoppoint_register(std::uint64_t control_register) {
        ...
        sdb::error::send("No remaining hardware debug register");
    }
}
...
sdb::process::cleanup_exited_threads(pid_t main_stop_id) {
    std::vector<pid_t> to_remove;
    std::optional<stop_reason> to_report;

    for (auto& [tid, thread] : threads_) {
        if (tid != main_stop_id and 
            ...
            if (tid == pid_) { // main thread exited
...
sdb::stop_reason sdb::process::step_instruction(std::optional<pid_t> otid) {
    ...
    // after stepping
    pid_t tid = otid.value_or(current_thread_);
...
std::optional<sdb::stop_reason> 
sdb::process::handle_signal(stop_reason reason, bool is_main_stop) {
    auto tid = reason.tid;

    // if thread was created and the signal caused the original stop
    if (reason.trap_reason and *reason.trap_reason == trap_type::clone and is_main_stop) {
        return std::nullopt;
    }

    if (is_attached_ and reason.reason == process_state::stopped) {
        if (!threads_.count(tid)) { // received signal for thread we are not tracking yet
...
sdb::stop_reason sdb::process::wait_on_signal(pid_t to_wait) {
    int wait_status;
    int options = __WALL;
    pid_t tid;
    if ((tid = waitpid(to_wait, &wait_status, options)) < 0) {
        error::send_errno("waitpid failed");
    }
    ...
    if (!final_reason) {
        resume(tid);
        return wait_on_signal(to_wait);
    }

    reason = *final_reason;
    auto& thread = threads_.at(to_wait);
    thread.reason = reason;
    thread.state = reason.reason;

    // check if any threads exited/terminated
    if (reason.reason == process_state::exited or reason.reason == process_state::terminated) {
        report_thread_lifecycle_event(reason);
        if (tid == pid_) {
            // main_thread exited == process terminated
...
void sdb::process::read_all_registers(pid_t tid) {
    ...
    if (ptrace(PTRACE_GETFPREGS, tid, nullptr, &get_registers().data_.i387) < 0) {
...
void sdb::process::write_gprs(
    const user_regs_struct& gprs,
    std::optional<pid_t> otid) {
...
std::vector<std::byte> sdb::process::read_memory_without_traps(virt_addr address, std::size_t amount) const {
    // replace int3 instructions in enabled sites with saved instruction
...
int sdb::process::set_hardware_stoppoint(virt_addr address, stoppoint_mode mode, std::size_t size) {
    ...
    // set debug register for all current threads
    for (auto& [tid, _] : threads_) {
