...
void sdb::breakpoint_site::enable() {
    ...
    if (is_hardware_) {
        ...
        saved_data_ = static_cast<std::byte>(data & 0xff); // only need 8 bits

        std::uint64_t int3 = 0xcc; // int3 instruction
        std::uint64_t data_with_int3 = ((data & ~0xff) | int3); // replaced saved data with int3 instruction
...
void sdb::breakpoint_site::disable() {
    ...
    if (is_hardware_) {
        ...
        std::uint64_t data = ptrace(PTRACE_PEEKDATA, process_->pid(), address_, nullptr);
        if (errno != 0)
            error::send_errno("Disabling breakpoint failed");
